...
const ButtonComponent = ({ name }) => {
    ...
    const filterCards = () => {
        history.push(`/content-manager/collectionType/${currentCollection.to}?page=1&pageSize=10&sort=${currentCollection.filterBy}:ASC&filters[$and][0][${currentCollection.from.split('.')[1]}][${currentCollection.filterBy}][$eq]=${modifiedData.title}`)
    }
